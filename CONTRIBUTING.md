# Contributing to Rally

Thank you for helping students find better opportunities. This repository is Rally's public community home; the application and reviewed data are maintained in [Rally-Source](https://github.com/ArmmyC/Rally-Source).

## Ways to help

- Report a listing, organizer, logo, deadline, status, or link correction with the [data correction form](https://github.com/ArmmyC/Rally/issues/new?template=data-correction.yml).
- Report a reproducible website problem with the [bug report form](https://github.com/ArmmyC/Rally/issues/new?template=bug_report.yml).
- Suggest a focused improvement with the [feature idea form](https://github.com/ArmmyC/Rally/issues/new?template=feature_request.yml).
- Open a documentation or community-workflow pull request when you can make the change directly.

## Evidence-first corrections

For opportunity or organizer corrections:

1. Link the official organizer, university, company, or programme page.
2. Explain exactly what is wrong and what should change.
3. Include the relevant Thai or English wording when useful.
4. Do not guess deadlines, eligibility, compensation, status, or application routes.
5. Do not upload personal data, credentials, tracking code, or copyrighted material you do not have permission to share.

Small, specific reports are easier to verify and publish. A reviewer may ask for more evidence before changing the directory.
